...
function GoogleMusicWebkit() {
  ...
  this.googleWebkit = gui.Window.open('https://play.google.com/music', {
  ...
  this.googleWebkit.on('loaded', function () {
    ...
    var loc = that.googleWebkit.window.location;
...
extend(GoogleMusicWebkit.prototype, {
  ...
  initMusic: function () {
    ...
    this.googleMusic = this.googleWebkit.window.googleMusic;
    if (!this.googleMusic) {
      // Bind to the window
      this.googleMusic = this.googleWebkit.window.googleMusic = new GoogleMusic(this.googleWebkit.window);
  ...
  destroyMusic: function () {
    ...
    delete this.googleWebkit.window.googleMusic;
  ...
  playPause: function () {
    ...
    if (this.googleMusic) {
      this.googleMusic.playback.playPause();
    }
  },
  next: function () {
    if (this.googleMusic) {
      this.googleMusic.playback.forward();
    }
  },
  previous: function () {
    if (this.googleMusic) {
      this.googleMusic.playback.rewind();
    }
  },
...
gmw.on('change:song', function (data) {
  tray.tooltip = [
    'Title: ' + data.title,
    'Artist: ' + data.artist,
    'Album: ' + data.album,
...
gmw.on('change:playback', function (playbackState) {
