...
googleWebkit.on('loaded', function () {
  ...
  if (loc.href.indexOf('//play.google.com/music') !== -1 && loc.hash !== '') {
    ...
    if (needToAddScripts) {
      ...
      needToAddScripts = false;

      // TODO: Add our scripts
      console.log('add scripts');
    }
  // If we have left Google Music (e.g. left Google Music), get ready to add scripts back
  } else {
    needToAddScripts = true;
  }
});
