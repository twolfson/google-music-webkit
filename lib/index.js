...
var playPauseMenuItem = new gui.MenuItem({
  label: 'Stopped',
  // disabled: true,
  click: function () {
    gmw.playPause();
...
trayMenu.append(playPauseMenuItem);
trayMenu.append(new gui.MenuItem({
  label: 'Next',
  click: function () {
    gmw.next();
...
gmw.on('playback:changed', function (playbackState) {
  // TODO: Move back to `Play/Pause` but with visual cue on icon
  if (playbackState === GoogleMusic.PLAYBACK.STOPPED) {
    // TODO: Consolidate this with defaults when we initialize playPauseMenuItem
    playPauseMenuItem.disabled = true;
    playPauseMenuItem.label = 'Stopped';
  } else {
    playPauseMenuItem.disabled = false;
    playPauseMenuItem.label = 'Play/Pause';
  }
});
